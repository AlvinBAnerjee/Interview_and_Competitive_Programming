...
classDiagram
    ...
    Shape <|.. Circle : Realization (implements)
    Car --> Engine : Association (field)
    Team o-- Player : Aggregation (shared has-a)
    House *-- Room : Composition (owned has-a)
    Client ..> Service : Dependency (uses)
```
